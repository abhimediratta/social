...
select match_result,json_agg(career.batting_score),count(*),round(avg(batting_score),2) as average, sum(career.batting_score),total_average, total_runs
from career, stats
where career.opposition = 'Australia'
group by career.match_result, stats.total_average, stats.total_runs

--Against Australia 100s/50s
with stats as (SELECT 
    CASE WHEN batting_score >= 100 then 1 else 0 END century,
    CASE WHEN batting_score >= 50 and batting_score < 100  THEN 1 ELSE 0 END  half_century,
    batting_score
...
select match_result,json_agg(batting_score),count(*),round(avg(batting_score),2) as average, sum(career.batting_score),total_average, total_runs
from career, stats
where ground in(select name from city where country = 'Australia' or country = 'New Zealand')
group by career.match_result, stats.total_average, stats.total_runs

with cities as (select distinct country from city)
select json_agg(career.batting_score), cities.country, sum(career.batting_score), count(career) as no_of_matches, round(avg(career.batting_score),2) as batting_average
from career, cities
